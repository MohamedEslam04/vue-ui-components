  ...
  try {
    ...
    execSync(
      `npx esbuild ${inputFiles.join(
        ' '
      )} --format=esm --outdir=${DST} --outbase=${SRC} --minify ${sharedOptions.join(' ')}`
    )
    execSync(
      `npx esbuild ${input} --format=esm --outfile=${DST}/${name}.esm.js --outbase=${SRC} --minify ${sharedOptions.join(
        ' '
      )}`
    )
    ...
    // Common JS builds
    execSync(
      `npx esbuild ${input} --format=cjs --outfile=${DST}/${name}.prod.cjs --minify --bundle ${sharedOptions.join(
        ' '
      )} ${process.argv.slice(2).join(' ')}`
    )
    execSync(
      `npx esbuild ${input} --format=cjs --outfile=${DST}/${name}.dev.cjs --bundle ${sharedOptions.join(
        ' '
      )} ${process.argv.slice(2).join(' ')}`
    )
