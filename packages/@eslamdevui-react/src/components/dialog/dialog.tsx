...
>(props: DialogProps<TTag>, ref: Ref<HTMLElement>) {
  ...
  let {
    id = `eslamdevui-dialog-${internalId}`,
    open,
    onClose,
    initialFocus,
    role = 'dialog',
  ...
  useInertOthers(inertOthersEnabled, {
    allowed: useEvent(() => [
      // Allow the eslamdevui-portal of the Dialog to be interactive. This
      // contains the current dialog and the necessary focus guard elements.
      internalDialogRef.current?.closest<HTMLElement>('[data-eslamdevui-portal]') ?? null,
    ]),
    disallowed: useEvent(() => [
      // Disallow the "main" tree root node
      mainTreeNode?.closest<HTMLElement>('body > *:not(#eslamdevui-portal-root)') ?? null,
    ]),
  })
...
function PanelFn<TTag extends ElementType = typeof DEFAULT_PANEL_TAG>(
  ...
  let internalId = useId()
  let { id = `eslamdevui-dialog-panel-${internalId}`, transition = false, ...theirProps } = props
...
function TitleFn<TTag extends ElementType = typeof DEFAULT_TITLE_TAG>(
  ...
  let internalId = useId()
  let { id = `eslamdevui-dialog-title-${internalId}`, ...theirProps } = props
