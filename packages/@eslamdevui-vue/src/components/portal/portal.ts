...
function getPortalRoot(contextElement?: HTMLElement | null) {
  ...
  let existingRoot = ownerDocument.getElementById('eslamdevui-portal-root')
  if (existingRoot) return existingRoot

  let root = ownerDocument.createElement('div')
  root.setAttribute('id', 'eslamdevui-portal-root')
...
export let Portal = defineComponent({
  ...
  setup(props, { slots, attrs }) {
    ...
    onUnmounted(() => {
      let root = ownerDocument.value?.getElementById('eslamdevui-portal-root')
      if (!root) return
