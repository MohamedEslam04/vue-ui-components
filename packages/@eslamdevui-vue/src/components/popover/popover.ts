...
import { useNestedPortals } from '../portal/portal'
...
export let PopoverButton = defineComponent({
  ...
  props: {
    ...
    id: { type: String, default: () => `eslamdevui-popover-button-${useId()}` },
  },
  inheritAttrs: false,
  setup(props, { attrs, slots, expose }) {
    ...
    let sentinelId = `eslamdevui-focus-sentinel-${useId()}`
    ...
    function handleFocus() {
      ...
      function run() {
        ...
        if (result === FocusResult.Error) {
          focusIn(
            getFocusableElements().filter((el) => el.dataset.eslamdevuiFocusGuard !== 'true'),
      ...
      return h(Fragment, [
        ...
        visible &&
          ...
          h(Hidden, {
            ...
            'data-eslamdevui-focus-guard': true,
            as: 'button',
            type: 'button',
            onFocus: handleFocus,
          }),
...
export let PopoverOverlay = defineComponent({
  ...
  setup(props, { attrs, slots }) {
    ...
    let id = `eslamdevui-popover-overlay-${useId()}`
...
export let PopoverPanel = defineComponent({
  ...
  props: {
    ...
    id: { type: String, default: () => `eslamdevui-popover-panel-${useId()}` },
  },
  inheritAttrs: false,
  setup(props, { attrs, slots, expose }) {
    let { focus } = props
    ...
    let beforePanelSentinelId = `eslamdevui-focus-sentinel-before-${useId()}`
    let afterPanelSentinelId = `eslamdevui-focus-sentinel-after-${useId()}`
        ...
        match(direction.value, {
          [TabDirection.Forwards]: () => {
            ...
            for (let element of combined.slice()) {
              if (element.dataset.eslamdevuiFocusGuard === 'true' || panel?.contains(element)) {
